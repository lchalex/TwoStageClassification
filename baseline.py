...
def main():
    ...
    if args.pretrained:
        logname = "baseline_{arch}_lr_{lr}_bs_{bs}_pretrained".format(arch=args.arch, lr=args.lr, bs=args.batch_size)
    else:
        logname = "baseline_{arch}_lr_{lr}_bs_{bs}".format(arch=args.arch, lr=args.lr, bs=args.batch_size)

    if args.evaluate:
        logname = "baseline_evaluate"
    ...
    model = get_model(arch=args.arch, num_class=len(class_df), pretrained=args.pretrained)
    model = model.to(args.device)
    train_transform, valid_transform = get_augmentation()
    train_dataset = BasicDataset(root=args.data, partition='train', transform=train_transform)
    valid_dataset = BasicDataset(root=args.data, partition='valid', transform=valid_transform)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in pbar:
        ...
        pbar.set_postfix(**{'lr': optimizer.param_groups[0]['lr'], 'loss': losses.avg, 'top1': top1.avg, 'top5': top5.avg})
